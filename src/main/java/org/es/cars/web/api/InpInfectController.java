...
@Controller
@Configuration
@RequestMapping(value = "/rest/inpInfect")
/**
 * Created by zhangw on 2017/6/12.
 * 医院感染类指标
 */
public class InpInfectController extends BaseController{
    Logger logger  =  LoggerFactory.getLogger(InpInfectController.class );
    ////初始化公共变量
     ...
     * 维度：时间、医院、科室
     */
    public String getInpInfectRate(@RequestBody String body) {
        logger.info("住院总感染率指标接口/rest/death/inpInfect 参数:" + body);
        String result = dealAllHasChildType(body, esQuery, deathRateAgg);
        logger.info("住院总感染率指标接口/rest/death/inpInfect 结果:" + result);
        return result;
    }
